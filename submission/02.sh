# Create a raw transaction that can be spent in 2 weeks time, assuming the current block is 25

# Amount of 20,000,000 satoshis to this address: 2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP 
# Use the UTXOs from the transaction below
# transaction=""

# Create inputs array for the transaction
inputs='[{"txid":"77f7def23b5f00d1d92c77e3b2e5a6f9d5957447d0865f8d69c5eebde8e92b0c8","vout":0}]'

# Create outputs array for the transaction
outputs='{"2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP":0.2}'

# Create the raw transaction with locktime
bitcoin-cli -regtest createrawtransaction "$inputs" "$outputs" 2041
